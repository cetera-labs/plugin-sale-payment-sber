...
function editor_text_SBER_tax_system_draw($field_def, $fieldvalue)
{
    ...
    Ext.create('Ext.form.ComboBox',{
		...
		store: new Ext.data.SimpleStore({
			fields: ['code', 'value'],
			data : [['', '- не установлено -'],['0', _('общая СН')], ['1', _('упрощенная СН (доходы)')], ['2', _('упрощенная СН (доходы минус расходы)')], ['3', _('единый налог на вмененный доход')], ['4', _('единый сельскохозяйственный налог')], ['5', _('патентная СН')],]
		}),
		defaultValue: '1'
    })
    <?
...
function editor_text_SBER_tax_draw($field_def, $fieldvalue)
{
    ...
    Ext.create('Ext.form.ComboBox',{
		...
		store: new Ext.data.SimpleStore({
			fields: ['code', 'value'],
			data : [['', '- не установлено -'],['0', _('без НДС')], ['1', _('НДС по ставке 0%')], ['2', _('НДС чека по ставке 10%')], ['3', _('НДС чека по ставке 18%')], ['4', _('НДС чека по расчетной ставке 10/110')], ['5', _('НДС чека по расчетной ставке 18/118')], ['6', _('НДС чека по ставке 20%')], ['7', _('НДС чека по расчетной ставке 20/120')]]
		}),
		defaultValue: '1'
    })
    <?
